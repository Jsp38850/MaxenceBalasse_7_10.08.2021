...
Post.getAll = (result) => {
  sql.query("SELECT posts.id, posts.user_id, posts.title, posts.content, posts.image, posts.adminApproved, posts.reported,posts.created_at, users.avatar, users.firstname, users.lastname, users.isAdmin FROM posts INNER JOIN users ON posts.user_id = users.userId ORDER BY posts.id DESC", (err, res) => {
    if (err) {
...
Post.delete = (postId, result) => {
  ...
  sql.query(`DELETE p,c FROM posts p INNER JOIN comments c ON p.id = c.post_id WHERE p.id = ${postId}`, (err, res) => {
    if (err) {
      console.log("error :", err);
      result(err, null);
      return;
...
Post.report = (postId, result) => {
  ...
  //Supprime l'approbation de l'administrateur au cas où l'administrateur a précédemment approuvé le message
